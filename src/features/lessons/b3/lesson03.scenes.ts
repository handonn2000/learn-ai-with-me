...
export function drawGrow(x: Ctx, { clock, w, h }: SceneApi) {
  x.clearRect(0, 0, w, h);
  const T = clock % 7.6;
  ...
  x.fillText('b = 3 · d = ' + done + ' → ' + tot + ' nút', 14, 20);
  if (T > 5.4) {
    const a = Math.min(1, (T - 5.4) / 0.7);
    x.fillStyle = 'rgba(11,13,18,' + a * 0.84 + ')'; x.fillRect(0, 0, w, h);
    x.globalAlpha = a; x.textAlign = 'center';
    x.font = '700 17px ' + MONO; x.fillStyle = '#FC6255'; x.fillText('b = 10, d = 12 → 10¹² nút', w / 2, h / 2 - 8);
    x.font = '11.5px ' + MONO; x.fillStyle = '#97A0B5'; x.fillText('≈ 13 ngày ở tốc độ 1 triệu nút/giây · ≈ 1 petabyte bộ nhớ', w / 2, h / 2 + 16);
    x.globalAlpha = 1;
  }
}

let stuckX: number | null = null;
export function drawHill(x: Ctx, { clock, w, h }: SceneApi) {
  x.clearRect(0, 0, w, h);
  const f = (t: number) =>
  ...
  x.fillText('không gian trạng thái →', w - 22, h - 14);
  x.textAlign = 'left'; x.fillText('↑ giá trị mục tiêu (−h)', 14, 18);
  ...
  x.font = '10.5px ' + MONO; x.fillStyle = '#83C167'; x.textAlign = 'center'; x.fillText('cực trị toàn cục', px(0.74), py(f(0.74)) - 12);
  const T = clock % 8;
  ...
  if (T > 3.6) {
    const a = 0.5 + 0.5 * Math.sin(clock * 5);
    ...
    x.fillText('KẸT — cực trị địa phương', px(bx) + 14, py(bY) - 32);
    x.font = '10px ' + MONO; x.fillStyle = '#97A0B5';
    x.fillText('muốn sang đỉnh cao hơn phải chấp nhận ĐI XUỐNG', px(0.52), h - 44);
  }
}
