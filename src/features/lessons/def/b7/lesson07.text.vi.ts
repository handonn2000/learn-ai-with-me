export const VI = {
  ...
  chapters: [
    {
      ...
      body: [
        "<strong class=\"def-term\">Hệ thống nguồn</strong> là nơi dữ liệu được sinh ra hoặc lưu giữ trước khi vào nền tảng dữ liệu: ứng dụng vận hành, cơ sở dữ liệu, API, tệp hoặc nhà cung cấp bên thứ ba. <strong class=\"def-term\">Ingestion</strong> thu nhận từ những nguồn này rồi chuyển vào nền tảng để xử lý tiếp; hiệu quả và độ tin cậy đều là yêu cầu của tầng này.",
    ...
    },
    {
      ...
      body: [
        "Tệp là một tập dữ liệu được lưu trong hệ thống máy tính. CSV, JSON, XML, Parquet, ảnh và PDF có thể nằm trên máy chủ nội bộ hoặc cloud storage. Một hệ thống xuất tệp mỗi giờ hay mỗi ngày; tác vụ ingestion nạp chúng theo lô vào lake hoặc warehouse. <strong class=\"def-term\">Micro-batch</strong> chia lô nhỏ hơn và chạy thường xuyên hơn để giảm thời gian chờ. Đây là mẫu phổ biến, không phải quy luật rằng mọi tệp đều phải chờ lịch dài.",
    ...
    },
    {
      ...
      body: [
        "<strong class=\"def-term\">CDC — Change Data Capture</strong> — theo dõi INSERT, UPDATE và DELETE, đưa phần thay đổi sang đích thay vì đọc lại toàn bảng ở mỗi lần chạy. Ba cách phổ biến là polling timestamp, trigger và đọc <strong class=\"def-term\">transaction log</strong>. Sơ đồ fan-out minh họa: source DB → transaction log → CDC → Elastic / Redis / Snowflake. Một luồng thay đổi có thể phục vụ tìm kiếm, cache và kho phân tích; mỗi đích vẫn cần cách áp dụng INSERT/UPDATE/DELETE phù hợp. Chúng khác nhau ở chỗ quan sát thay đổi và ở chi phí đặt lên nguồn.",
    ...
    },
    {
      ...
      body: [
        "Ban đầu mỗi đội nhận một bản xuất hàng ngày từ production DB. Khi lưu lượng tăng, các truy vấn xuất dữ liệu cạnh tranh với giao dịch trực tiếp; báo cáo đến muộn và các kết nối riêng lẻ khó bảo trì. <strong class=\"def-term\">Kafka</strong> cung cấp một nhật ký sự kiện dùng chung, tách nhịp sinh dữ liệu khỏi nhịp đọc của các ứng dụng.",
    ...
    },
    {
      ...
      body: [
        "<strong class=\"def-term\">Topic</strong> là tên logic tổ chức dòng bản ghi. Có thể đặt theo loại sự kiện, như user_created; theo thực thể/miền, như orders; hoặc tách môi trường dev / test / prod. Chọn tên cũng là chọn hợp đồng: ai được ghi, schema nào được chấp nhận và ai được đọc. Tên có chữ prod không tự tạo ranh giới bảo mật.",
      ...
      cards: [
        {
          ...
          text: "Chọn tab dư consumer: 3 partition, 4 consumer và 6 sự kiện. Xem Analytics tiến lên khi Fraud còn đứng yên, rồi Fraud theo kịp. Mỗi nhóm lưu mốc riêng; vòng minh họa mới mở lại ví dụ."
        }
      ],
      table: {
        headers: [],
      ...
      cards: [
        {
          ...
          text: "Xem INSERT → UPDATE → DELETE lặp lại, rồi đổi cách bắt CDC bằng các tab. So lịch sử giữ theo thời gian, dung lượng và compaction. Mô hình nhỏ có giả định rõ; trạng thái cuối đúng không chứng minh mọi sự kiện trung gian được giữ lại."
        }
      ],
      table: {
        headers: [],
  ...
  lab: {
    ...
    "routingSteps": [
        ...
        "Cả hai nhóm đã tới cuối log. Vòng tiếp theo chạy lại minh họa này."
    ],
