...
export const EN = {
  ...
  chapters: [
    {
      ...
      body: [
        "A <strong class=\"def-term\">source system</strong> is where data originates or is stored before entering the data platform: an operational application, database, API, file, or third-party provider. <strong class=\"def-term\">Ingestion</strong> collects from these sources and delivers into the platform for further processing; both efficiency and reliability belong to this layer.",
    ...
    },
    {
      ...
      body: [
        "<strong class=\"def-term\">CDC — Change Data Capture</strong> — tracks INSERT, UPDATE, and DELETE and sends changes downstream instead of rereading the whole table on every run. Three common approaches are timestamp polling, triggers, and <strong class=\"def-term\">transaction-log</strong> capture. The illustrated fan-out is source DB → transaction log → CDC → Elastic / Redis / Snowflake. One change stream can serve search, caching, and analytical storage; each target still needs appropriate INSERT/UPDATE/DELETE handling. They differ in where they observe changes and in the cost they impose on the source.",
      ...
      cards: [
        {
          ...
          text: "Choose the extra-consumer tab: 3 partitions, 4 consumers, and 6 events. Watch Analytics advance while Fraud stays put, then Fraud catch up. Each group saves its own progress; a new animation loop starts a fresh example."
        }
      ],
      table: {
        headers: [],
    ...
    },
    {
      ...
      body: [
        "<strong class=\"def-term\">At-most-once</strong> accepts that processing might never happen, without retrying under the illustrated policy. The consumer commits before processing: a crash after commit but before the effect causes the next run to skip the record. Lower-value telemetry/logging sometimes accepts a few lost points instead of retry costs, but that decision must follow business requirements.",
      ...
      cards: [
        {
          ...
          text: "Watch INSERT → UPDATE → DELETE repeat, then switch CDC methods using the tabs. Compare history retained by time, size, and compaction. These small models have explicit assumptions; a correct final state does not prove every intermediate event was retained."
        }
      ],
      table: {
        headers: [],
  ...
  lab: {
    ...
    "routingSteps": [
        ...
        "Both groups have reached the log end. The next loop restarts this illustration."
    ],
