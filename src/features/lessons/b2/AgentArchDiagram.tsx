...
import { ARCH_LEVELS, LEARNING_PARTS } from '@/content/courses/csc14003/agents-data.js';
  ...
  return (
    <>
      ...
      <text x={726} y={200} fontFamily="var(--font-serif)" fontSize={12} fill="var(--muted)" textAnchor="middle"
        transform="rotate(90 726 200)">Môi trường</text>
      {children}
    </>
  );
}
...
function Diagram14({ n }: { n: number }) {
  ...
  const centers = [
    { id: 'now', y: 70, from: 1, label: ['Thế giới đang', 'thế nào'] },
    { id: 'next', y: 145, from: 3, label: ['Sẽ ra sao nếu', 'tôi làm A'] },
    { id: 'happy', y: 220, from: 4, label: ['Làm vậy tôi', 'sướng cỡ nào'] },
    { id: 'act', y: 300, from: 1, label: ['Vậy giờ', 'nên làm gì'] },
  ];
  const CX = 600, CW = 170, CH = 42, CL = CX - CW / 2;
  const visible = centers.filter((c) => n >= c.from);

  const pills = [
    { id: 'state', y: 70, from: 2, to: 9, label: ['Trạng thái'] },
    { id: 'evolve', y: 112, from: 2, to: 9, label: ['Thế giới tự biến đổi ra sao'] },
    { id: 'myact', y: 154, from: 2, to: 9, label: ['Hành động của tôi làm gì'] },
    { id: 'goals', y: 210, from: 3, to: 9, label: ['Mục tiêu'] },
    { id: 'util', y: 252, from: 4, to: 9, label: ['Độ thỏa dụng'] },
    { id: 'rules', y: 300, from: 1, to: 2, label: ['Luật điều kiện–hành động'] },
  ];
  ...
  return (
    <Shell>
      ...
      <text x={600} y={44} textAnchor="middle" fontFamily="var(--font-serif)" fontSize={13} fill="var(--muted)">Cảm biến</text>
      <text x={600} y={366} textAnchor="middle" fontFamily="var(--font-serif)" fontSize={13} fill="var(--muted)">Bộ chấp hành</text>
...
function Diagram5() {
  return (
    <Shell>
      <text x={600} y={62} textAnchor="middle" fontFamily="var(--font-serif)" fontSize={13} fill="var(--muted)">Cảm biến</text>
      <text x={600} y={360} textAnchor="middle" fontFamily="var(--font-serif)" fontSize={13} fill="var(--muted)">Bộ chấp hành</text>
      <Arrow d="M700,58 L634,58" on />
      <Arrow d="M632,356 L700,356" on />

      <text x={170} y={26} textAnchor="middle" fontFamily={FONT} fontSize={10} fill="var(--muted)">Chuẩn hiệu năng</text>
      <Arrow d="M170,32 L170,44" on />

      <Box x={90} y={48} w={160} h={44} label={['Bộ phê bình', 'Critic']} on fresh />
      <Box x={90} y={175} w={160} h={50} label={['Bộ học', 'Learning element']} on fresh />
      <Box x={440} y={175} w={200} h={50} label={['Bộ thi hành', 'Performance element']} on fresh={false} />
      <Box x={90} y={300} w={160} h={50} label={['Bộ sinh vấn đề', 'Problem generator']} on fresh />

      <Arrow d="M575,58 L254,66" on label="tri giác" lx={410} ly={54} />
      <Arrow d="M170,92 L170,171" on label="phản hồi" lx={210} ly={136} />
      <Arrow d="M254,188 L436,188" on label="sửa đổi" lx={345} ly={180} />
      <Arrow d="M436,214 L254,214" on label="tri thức" lx={345} ly={230} />
      <Arrow d="M170,225 L170,296" on label="mục tiêu học" lx={222} ly={266} />
  ...
  return (
    <div className="card" style={{ padding: '20px 22px 22px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
        <span style={{ fontWeight: 600, fontSize: 15 }}>Năm nấc, một sơ đồ</span>
        <span style={{ color: 'var(--faint)', fontSize: 12.5, fontStyle: 'italic' }}>
          bấm 1 → 5, hộp vàng là thứ nấc đó vừa thêm vào
        </span>
      </div>
      ...
      <div style={{ marginTop: 14, borderRadius: 12, border: '1px solid var(--border-2)', overflow: 'hidden' }}>
        <svg viewBox="0 0 760 400" style={{ width: '100%', display: 'block' }} role="img"
          aria-label={`Sơ đồ kiến trúc agent nấc ${n}: ${lv.name}`}>
          {n === 5 ? <Diagram5 /> : <Diagram14 n={n} />}
        </svg>
      </div>

      <div style={{ marginTop: 14, display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
        <span className="mono" style={{ fontSize: 11, color: lv.color, fontWeight: 700 }}>NẤC {lv.n}</span>
      ...
      {lv.limit ? (
        <div className="panel-inner" style={{ marginTop: 12, borderColor: 'color-mix(in srgb, var(--red) 28%, transparent)' }}>
          <span className="mono" style={{ fontSize: 10, color: 'var(--red)', letterSpacing: '0.7px' }}>CHỖ NÓ CHỊU THUA — VÀ LÀ LÝ DO CÓ NẤC SAU</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(260px,100%),1fr))', gap: 12, marginTop: 14 }}>
          {(LEARNING_PARTS as { k: string; en: string; color: string; what: string; taxi: string }[]).map((p) => (
            <div key={p.k} className="panel-inner">
              ...
              <Html as="p" t={'<em>Với chiếc taxi:</em> ' + p.taxi} style={{ color: 'var(--muted)', fontSize: 12.5, lineHeight: 1.6, margin: '7px 0 0' }} />
            </div>
          ))}
        </div>
      )}
