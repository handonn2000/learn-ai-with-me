...
try {
  ...
  check('implemented DEF sessions 7–12 link to lessons; no active DEF part tests', () => {
    assert(!vi.roadmapOnly && !en.roadmapOnly);
    assert.equal(vi.hasPartTests, false);
    assert.equal(en.hasPartTests, false);
    for (const c of [vi, en]) for (const s of c.sessions) { assert.equal(s.lessonPath, ['b7', 'b8', 'b9', 'b10', 'b11', 'b12'].includes(s.id) ? `/courses/data-engineering-foundation/lessons/${s.id}` : undefined); assert.deepEqual(s.links, []); }
